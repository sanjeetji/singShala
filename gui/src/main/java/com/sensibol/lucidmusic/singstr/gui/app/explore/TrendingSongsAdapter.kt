...
internal class TrendingSongsAdapter @Inject constructor() : RecyclerView.Adapter<TrendingSongsAdapter.ExploreSongVH>() {
    ...
    fun addToTrendingSongsView(trendingSongs: List<ExploreSongView>){
        trendingSongViews = if(trendingSongViews.isEmpty()){
            trendingSongs
        } else{
            val result: MutableList<ExploreSongView> = mutableListOf()
            result.addAll(trendingSongViews)
            result.addAll(trendingSongs)
            result
        }
